...
Time* array[MAXTAM+1];   // Elementos da pilha 
...
void inserir(Time* x) {
   ...
   ultimo = (ultimo + 1) % MAXTAM;
	...
	for(int i = 0; i < ultimo+1; i++){
		if(array[i]->fundacaoAno != NULL && (floor(log10(abs(array[i]->fundacaoAno))) + 1) == 4){
			//printf("%d\n", array[i]->fundacaoAno);
			m += array[i]->fundacaoAno;
			//printf("%i\n", m);
		}
