...
int maior()
{
    int maiorE = array[0]->paginaTam;
    for(int i = 1; i < n; i++)
    {
        if(maiorE < array[i]->paginaTam)
            maiorE = array[i]->paginaTam;
    }
    return maiorE;
}
...
void RadixSort()
{
    Time* bucket[10][10];
    int bucket_count[10];
    int i, j, k, remainder, NOP=0, divisor=1, maiorE, pass;
    ...
    for(pass = 0; pass < NOP; pass++)
    {
        ...
        for(i = 0; i < n; i++)
        {
            remainder = (array[i]->paginaTam / divisor) % 10;
