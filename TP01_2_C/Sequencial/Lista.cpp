 ...
 */
bool pesquisar(char* pesquisa) {
   bool retorno = false;
   int i;

   for (i = 0; i < n && retorno == false; i++) {
      retorno = (strstr(array[i]->nome, pesquisa) != NULL);
   }
   return retorno;
}
...
int main()
{
	...
	for(int i = 0; i < oper; i++)
	{
		...
		if(contains(operacao2, "II")){
			for(int i = 3; i < strlen(operacao2); i++){
				operacao[j] = operacao2[i];
				j++;
			}
			lerHtml(operacao, t);
			inserirInicio(t);
		}else if(contains(operacao2, "IF")){
			for(int i = 3; i < strlen(operacao2); i++){
				operacao[j] = operacao2[i];
				j++;
			}
			lerHtml(operacao, t);
			inserirFim(t);
		}else if(contains(operacao2, "R*")){
			for(int i = 3; i < strlen(operacao2); i++){
				operacao[j] = operacao2[i];
				j++;
			}
			sscanf(operacao, "%d", &pos);
			printf("(R) %s\n", remover(pos)->nome);
		}else if(contains(operacao2, "RI")){
			printf("(R) %s\n", removerInicio()->nome);
		}
		else if(contains(operacao2, "RF")){
			printf("(R) %s\n", removerFim()->nome);
		}else if(contains(operacao2, "I*")){
			for(int i = 6; i < strlen(operacao2); i++){
				operacao[j] = operacao2[i];
				j++;
			}
			operacao3[0] = operacao2[3];
			operacao3[1] = operacao2[4];

			sscanf(operacao3, "%d", &pos);
			lerHtml(operacao, t);
			inserir(t, pos);
		}
