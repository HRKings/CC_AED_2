...
bool pesquisar(char* pesquisa) {
   ...
   for (i = 0; i < n && retorno == false; i++) {
	//printf("%s == %s\n", pesquisa, array[i]->nome);
      retorno = (strcmp(pesquisa, array[i]->nome) == 0);
   }
   return !retorno;
}
...
int main()
{
	...
	fgets(arquivo2, 128, stdin);

		if (arquivo2[strlen (arquivo2) - 1] == '\n'){
			arquivo2[strlen (arquivo2) - 1] = '\0';
		}

	while (arquivo != NULL && strcmp(arquivo2, "FIM") != 0)
	{
		bool pesquisa = pesquisar(arquivo2);
