...
class ListaTime {
	...
	public void selecaoDesempate() {
		for (int i = 1; i < (n - 1); i++) {
			comparacoes++;
		   if(array[i].getPaginaTam() == array[i - 1].getPaginaTam()){
			comparacoes++;
			   for(int j = 0; j < 10; j++){
				comparacoes++;
				   if(array[i].getNome().charAt(j) != array[i - 1].getNome().charAt(j)){
					comparacoes++;
					   if(array[i].getNome().charAt(j) < array[i - 1].getNome().charAt(j)){
						comparacoes++;
						swap(i, i-1);
...
class Selecao_Parcial {
	...
	public static void main(String[] args) {
		...
		lista.selecao();
		lista.selecaoDesempate();
		long fim = new Date().getTime();

		Arq.openWriteClose("66978_selecaoParcial.txt", "UTF-8", "66978\t" + (fim-comeco)/1000.0 + "\t" + lista.comparacoes + "\t" + lista.mov);
