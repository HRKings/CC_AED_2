...
class ListaTime {
	...
	public void insercaoEmpate() {
		for (int i = 1; i < n; i++) {
			comparacoes++;
			int j = i - 1;

			if(array[i].getFundacaoAno() == array[j].getFundacaoAno()){
				comparacoes++;
				for(int k = 0; k < 10; k++){
					comparacoes++;
					try {
						if(array[i].getNome().charAt(k) < array[j].getNome().charAt(k)){
							comparacoes++;
							mov += 3;
							Time temp = array[i];
							     array[i] = array[j];
...
class Insercao {
	...
	public static void main(String[] args) {
		...
		lista.insercao();
		lista.insercaoEmpate();
		long fim = new Date().getTime();

		Arq.openWriteClose("66978_insercao.txt", "UTF-8", "66978\t" + (fim-comeco)/1000.0 + "\t" + lista.comparacoes + "\t" + lista.mov);
