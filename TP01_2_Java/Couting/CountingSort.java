...
class ListaTime {
	...
	public int menorTamanho(String s1, String s2) {
		comparacoes++;
	...
	public int getMaior() {
		...
		for (int i = 0; i < n; i++) {
			comparacoes++;
			if (maior < array[i].getCapacidade()) {
				comparacoes++;
	...
	public void countingsort() {
		...
		for (int i = 0; i < n; count[array[i].getCapacidade()]++, comparacoes++, i++);

		// Agora, o count[i] contem o numero de elemento menores ou iguais a i
		for (int i = 1; i < count.length; count[i] += count[i - 1], comparacoes++, i++);

		// Ordenando
		for (int i = n - 1; i >= 0; ordenado[count[array[i].getCapacidade()] - 1] = array[i], count[array[i].getCapacidade()]--, movimentacoes++, comparacoes++, i--);

		// Copiando para o array original
		for (int i = 0; i < n; array[i] = ordenado[i], comparacoes++, movimentacoes++, i++);
	}
...
class CountingSort {
	...
	public static void main(String[] args) {
		...
		Arq.openWriteClose("66978_countingsort.txt", "UTF-8", "66978\t" + (fim-comeco)/1000.0 + "\t" + lista.comparacoes + "\t" + lista.movimentacoes);
