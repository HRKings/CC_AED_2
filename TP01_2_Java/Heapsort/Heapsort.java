...
class ListaTime {
	...
	public void constroi(int tamHeap) {
		try {
			for (int i = tamHeap; i > 1 && array[i].getPaginaTam() > array[i / 2].getPaginaTam() ; i /= 2) {
				comparacoes++;
				swap(i, i / 2);
			}
		} catch (Exception e) {
	...
	public void reconstroi(int tamHeap) {
		...
		while (i <= (tamHeap / 2)) {
			comparacoes++;
			try {
				if (array[2 * i].getPaginaTam()  > array[2 * i + 1].getPaginaTam()  || 2 * i == tamHeap) {
					comparacoes++;
					filho = 2 * i;
				} else {
					filho = 2 * i + 1;
				...
				if (array[i].getPaginaTam()  < array[filho].getPaginaTam() ) {
					comparacoes++;
					swap(i, filho);
					i = filho;
				} else {
...
class Heapsort {
	...
	public static void main(String[] args) {
		...
		Arq.openWriteClose("66978_heapsort.txt", "UTF-8", "66978\t" + (fim-comeco)/1000.0 + "\t" + lista.comparacoes + "\t" + lista.mov);
