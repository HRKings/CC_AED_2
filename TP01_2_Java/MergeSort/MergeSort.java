...
class ListaTime {
	...
	public void merge(Time[] a, Time[] l, Time[] r, int left, int right) {

		int i = 0, j = 0, k = 0;
		while (i < left && j < right) {
			comparacoes++;
			if (l[i].getPaginaTam() <= r[j].getPaginaTam()) {
				a[k++] = l[i++];
				mov++;
				comparacoes++;
			} else {
	...
	public  void mergeSortEngine(Time[] a, int m) {
		if (m < 2) {
			comparacoes++;
			return;
		}
		...
		mergeSortEngine(l, mid);
		mergeSortEngine(r, m - mid);

		merge(a, l, r, mid, m - mid);
	}

	public void mergeSort(){
		mergeSortEngine(array, array.length - 1);
	}
...
class MergeSort {
	...
	public static void main(String[] args) {
		...
		Arq.openWriteClose("66978_mergesort.txt", "UTF-8", "66978\t" + (fim-comeco)/1000.0 + "\t" + lista.comparacoes  + "\t" + lista.mov);
